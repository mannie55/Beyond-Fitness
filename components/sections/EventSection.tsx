...
export default function EventSection() {
  ...
  const handleDragEnd = (e: MouseEvent | TouchEvent | PointerEvent, { offset, velocity }: PanInfo) => {
    ...
    if (swipe < -swipeThreshold || offset.x < -75) {
      if (activeTab === "on-tour") {
        setDirection(1);
        setActiveTab("pink-walk");
      }
    } else if (swipe > swipeThreshold || offset.x > 75) {
      if (activeTab === "pink-walk") {
        setDirection(-1);
        setActiveTab("on-tour");
      }
    }
  };

  const handleTabClick = (newTab: EventTab) => {
    if (newTab === activeTab) return;
    setDirection(newTab === "pink-walk" ? 1 : -1);
    setActiveTab(newTab);
  };
        ...
        <div className="relative w-full h-full overflow-hidden">
          <AnimatePresence mode="wait" custom={direction}>
            <motion.div
              ...
              className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 lg:gap-16 items-center"
            >
              ...
              <div className="lg:col-span-7 w-full pointer-events-none">
