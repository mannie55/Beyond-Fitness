...
export default function FirstTimersCard() {
  ...
  return (
    <div className="w-full flex flex-col justify-between items-stretch p-5 sm:p-6 md:p-8 gap-5 sm:gap-6 md:gap-8 bg-[#0D0B05] rounded-none border border-white/10 font-sans h-full transition-all hover:border-white/30">
      ...
      <div className="flex flex-col gap-4 sm:gap-5 md:gap-6">
          ...
          <button
            ...
            className={`flex-1 py-1.5 sm:py-2 font-bold text-xs sm:text-sm transition-all text-center uppercase tracking-wider ${
              activeTab === "firstTimer"
                ? "bg-white text-[#0D0B05]"
                : "bg-transparent text-white/70 hover:bg-white/5"
            }`}
          >
            First Timer
          </button>
          <div className="w-[0.0625rem] h-full bg-white/20 self-stretch" />
          <button
            type="button"
        ...
        {/* Price Container */}
        <div className="flex flex-col items-start gap-1">
          <span className="text-[clamp(1.875rem,5vw,2.75rem)] xl:text-[3.125rem] font-bold leading-[1.1] tracking-tight text-white whitespace-nowrap">
            {current.price}
          </span>
        ...
        {/* Action Button */}
        <Button variant="secondary" theme="dark" width="w-full">
          GET STARTED
        </Button>
